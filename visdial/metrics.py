...
def evaluateMetric(ranks, metric):
    ...
    if metric == 'r10':
        # ranks = ranks.reshape(-1)
        ranks = ranks.reshape(-1)
        # return 100*torch.sum(ranks <= 10).item()/float(ranks.size(0))
        return 100 * (ranks <= 10).sum() / float(ranks.shape[0])
    if metric == 'mean':
        # ranks = ranks.reshape(-1).float()
        ranks = ranks.reshape(-1).astype(float)
        return ranks.mean()
    if metric == 'mrr':
        # ranks = ranks.reshape(-1).float()
        ranks = ranks.reshape(-1).astype(float)
        # return torch.reciprocal(ranks).mean().item()
        return (1 / ranks).mean()
...
def scores_to_ranks(scores: torch.Tensor):
    ...
    scores = scores.reshape(-1, num_options)
    ...
    for i in range(ranked_idx.size(0)):
        for j in range(num_options):
            ranks[i,ranked_idx[i][j].item()] = j
    # convert from 0-99 ranks to 1-100 ranks
    ranks += 1
    ranks = ranks.reshape(batch_size, num_rounds, num_options)
    return ranks
...
class NDCG(object):
    ...
    def observe(self,
        ...
        for batch_index in range(batch_size):
            ...
            self.ndcg_vals.append(dcg.item()/best_dcg.item())
