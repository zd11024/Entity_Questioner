"""
at each time step, we concatenate the presiously generated word embedding with the selected entity embedding as input.
"""
import torch
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.d_entity_emb = d_entity_emb

        # Modules
        self.rnn = nn.LSTM(
            self.embedSize + self.d_entity_emb,
            self.rnnHiddenSize,
            self.numLayers,
            batch_first=True,
    ...
    def forward(self, encStates, inputSeq, entity_emb):
        '''
        ...
        if inputSeq is not None:
            inputSeq = self.wordEmbed(inputSeq)  # (n_batch, n_seq, d_word_embed)
            n_seq = inputSeq.size(1)
            inputSeq = torch.cat([inputSeq, entity_emb.unsqueeze(1).repeat(1, n_seq, 1)], dim=2)  # (n_batch, n_seq, d_word_embed+2*nhid)
            outputs, _ = self.rnn(inputSeq, encStates)
            outputs = F.dropout(outputs, self.dropout, training=self.training)  # (n_batch, n_seq, d_hidden)
            ...
            logProbs = flatLogProbs.reshape(outputSize[0], outputSize[1], -1)  # (n_batch, n_seq, n_vocab)
    ...
    def forwardDecode(self,
        ...
        if inference == 'greedy' and beamSize > 1:
            ...
            return self.beamSearchDecoder(encStates, entity_emb, beamSize, maxSeqLen)
        ...
        for t in range(maxLen - 1):
            ...
            if inference == 'sample' and run_mcts:
                ...
                if t != maxLen-1:
                    ...
                    self.saved_log_probs.append(logProb.gather(1, sample.reshape(-1, 1)))
                ...
                self.saved_log_probs.append(categorical_dist.log_prob(sample))  # list of tensor(n_batch,)
                sample = sample.unsqueeze(-1)
        ...
        samples = torch.cat(gen_samples, 1)  # (n_batch, seq_len)
    ...
    def beamSearchDecoder(self, initStates, entity_emb, beamSize, maxSeqLen):
        '''
        ...
        for t in range(maxLen - 1):  # Beam expansion till maxLen]
            if t == 0:
                ...
                hiddenStates = [
                    x.reshape(self.numLayers, -1, self.rnnHiddenSize)
                    for x in hiddenStates
                ]
                ...
                emb = torch.cat([emb, entity_emb.unsqueeze(1).repeat(1, beamSize, 1)], dim=2)
                # emb has shape (batchSize, beamSize, embedSize)
                output, hiddenStates = self.rnn(
                    emb.reshape(-1, 1, self.embedSize+self.d_entity_emb), hiddenStates)
                ...
                logProbsCurrent = logProbsCurrent.reshape(batchSize, beamSize,
                                                       self.vocabSize)
                ...
                logProbs = logProbs.reshape(batchSize, -1)
                ...
                tokensArray = tokensArray.reshape(batchSize, -1)
                backIndexArray = backVector.unsqueeze(2).\
                                repeat(1,1,self.vocabSize).reshape(batchSize,-1)
                ...
                hiddenCurrent = hiddenCurrent.reshape(
                    num_layers, batchSize, beamSize, rnnHiddenSize)
                cellCurrent = cellCurrent.reshape(
                    num_layers, batchSize, beamSize, rnnHiddenSize)
                ...
                hiddenCurrent = hiddenCurrent.reshape(*original_state_size)
                cellCurrent = cellCurrent.reshape(*original_state_size)
    ...
    def get_mcts_samples(self, initStates, beamSize, maxLen, startTokens):
        ...
        for t in range(maxLen):  # Beam expansion till maxLen
            if t == 0:
                ...
                emb = self.wordEmbed(beamTokensTable[:, :, t - 1])
                # emb has shape (batchSize, beamSize, embedSize)
                output, hiddenStates = self.rnn(emb.reshape(-1, 1, self.embedSize), hiddenStates)
                # output has shape (batchSize*beamSize, 1, rnnHiddenSize)
                scores = self.outNet(output.squeeze())
                logProbsCurrent = self.logSoftmax(scores)
                # Explicitly removing padding token (index 0) and <START> token
                ...
                samples = torch.cat(samples, 1)
                samples = samples.reshape(batchSize, beamSize)
